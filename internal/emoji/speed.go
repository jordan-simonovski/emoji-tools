...
func runSpeed(args []string) error {
	...
	for i := range fr {
		fr[i] = cloneRGBA(base)
		drawSpeedLines(fr[i], rng, *dirn, *lines)
	}
...
// drawSpeedLines streaks n anime speed lines across f. dirn is speed's -direction:
// "left" fills the left side with lines pointing right, and so on.
func drawSpeedLines(f *image.RGBA, rng *rand.Rand, dirn string, n int) {
